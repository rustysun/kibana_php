...
class_exists('ElasticSearchClient', FALSE) or require(__DIR__ . DIRECTORY_SEPARATOR . 'ElasticSearchClient.php');
...
class LogstashLoader {
    ...
    public function __construct($config = []) {
        ...
        $this->esClient = new ElasticSearchClient($config['es_server']);
    }
        ...
        if ($req) {
            ...
            switch ($req->mode) {
                case 'csv':
                    ...
                    header('Content-Disposition: attachment; filename=kibana_' . implode('_', $req->fields) . '-' . time() . '.csv');
                    echo $this->csvFeed($req, $return);
                    break;
                case 'rss':
                    echo $this->rssFeed($req, $return);
                    break;
                default:
    ...
    protected function collectInput() {
        ...
        if ($_GET['page']) {
            ...
            $req->interval = (isset($_GET['interval'])) ? self::roundInterval($_GET['interval']) : '10m';
    ...
    protected function buildQuery($req) {
        ...
        $query = [
            ...
            'size'  => $this->config['results_per_page'],
            'sort'  => ['@timestamp' => ['order' => 'desc']],
        ];
        $filter_string = ($this->config['filter_string'] == "") ? "" : " AND " . $this->config['filter_string'];
        ...
        $search = ($req->search == "") ? "*" . $filter_string : "(" . $req->search . ")" . $filter_string;
        if ($search == "*") {
            $query['query']['filtered']['query']['match_all'] = new stdClass;
        } else {
            $query['query']['filtered']['query']['query_string'] = [
                'query' => [
                    'default_field' => $this->config['primary_field'],
                ],
            ];
        }
        ...
        if ($time != '') {
            ...
            if ($this->config['smart_index']) {
                $this->index = implode(',', $this->getIndicesByTime($time->from, $time->to));
            }
        }
        ...
        switch ($req->mode) {
            ...
            case 'mean':
                unset($query->sort);
                $query['size'] = 0;
                $query['aggs']['statistics'] = [
                    'stats' => [//统计总数、总和、最值、均值
                                'field' => $req->analyze_field,
                    ],
                ];
                break;
            case 'rss':
    ...
    protected function processQuery($req, $query) {
        ...
        if (strpos($req->mode, 'graph') !== FALSE) {
            ...
            $interval = (strtotime($req->time->to) - strtotime($req->time->from)) * 10;
            $return->graph->interval = $interval;
        }
        ...
        if ($req->mode != 'mean' && strpos($req->mode, 'graph') === FALSE) {
            ...
            if (isset($result->error)) {
                die(json_encode($query, JSON_UNESCAPED_UNICODE));
            } else {
                $return->hits = $result->hits->total;
            }
            $i = 1;
            while ($return->hits <= ($req->offset + $slice) && $i < sizeof($index_array)) {
        ...
        if (isset($result->aggregations->histo1)) {
            $return->graph->data = $result->aggregations->histo1->buckets;
        }

        switch ($req->mode) {
            case 'analyze':
                $result->hits->hits = array_slice($result->hits->hits, 0, $slice);
                $return = $this->analyzeField($req, $return, $result);
                break;
            ...
            default:
                $result->hits->hits = array_slice($result->hits->hits, 0, $slice);
                //$base_fields = array_values(array_unique(array_merge(['@message'], $this->config['default_fields'])));
                //$return->all_fields = array_values(array_unique(array_merge(['@message'], $this->config['default_fields'])));
                $base_fields = array_values(array_unique($this->config['default_fields']));
                $return->all_fields = array_values(array_unique($this->config['default_fields']));
                $return->page_count = count($result->hits->hits);
                $i = 0;
                foreach ($result->hits->hits as $hit_key => $hit) {
                    $i++;
                    $hit_id = $hit->{'_id'};
                    $hit->fields = $hit->{'_source'};
                    $return->results[$hit_id]['@cabin_time'] = date('m/d H:i:s', strtotime($hit->fields->{'@timestamp'}));
                    $return->results[$hit_id]['@timestamp'] = $hit->fields->{'@timestamp'};
                    /*
                    //TODO:all_fields
                    ...
                    */
                    ...
                    foreach ($base_fields as $field) {
                        $return->results[$hit_id][$field] = $hit->fields->{$field};
                    }
                    unset($result->hits->hits[$i]);
                }
            //sort($return->all_fields);
        }
        if (sizeof($req->fields) == 0) {
            $req->fields = ['@message'];
        }
     ...
     */
    protected function rssFeed($req, $return) {
        ...
        $pTitleText = $pDom->createTextNode('Kibana: ' . $e_query);
        $pLinkText = $pDom->createTextNode('http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['REQUEST_URI']) . '/#' . base64_encode(json_encode($req)));
        $pDescText = $pDom->createTextNode('An event search for: ' . $e_query . '. Showing fields ' . implode(', ', array_filter($req->fields)) . " in the title.");
        ...
        foreach ($return->results as $result) {
            ...
            $pTitle = $pDom->createElement('title');
            $pLink = $pDom->createElement('pubDate', date('r', strtotime($result['@timestamp'])));
            $pDesc = $pDom->createElement('description');
     ...
     * @return string Response to request
     */
    protected function csvFeed($req, $return) {
        if (sizeof($req->fields) < 1) {
            $req->fields = ['@message'];
        }
        $csv = [
            'timestamp' . $this->config['export_delimiter'] . implode($this->config['export_delimiter'], $req->fields),
        ];
        foreach ($return->results as $result) {
            $csv_line = [];
            array_push($csv_line, date('Y-m-d H:i:s', strtotime($result['@timestamp'])));
            foreach ($req->fields as $field) {
                if (is_array($result[$field])) {
            ...
            array_push($csv, implode($this->config['export_delimiter'], $csv_line));
            unset($csv_line);
        }
     ...
     * @return object Response to request
     */
    protected function analyzeField($req, $return, $result) {
        $field = self::canonicalFieldName($req->analyze_field);
        if (!isset($return->analysis)) {
            $return->analysis = new stdClass;
        }
        ...
        arsort($analyze);
        $analyze = array_slice($analyze, 0, $this->config['analyze_show'], TRUE);
        $final = [];
     ...
     *
     * @param array $query Search API query
     *
    ...
    function esQuery($query) {
        ...
        if ($this->config['type'] != "") {
            $url = "http://" . $this->config['elasticsearch_server'] . "/{$this->index}/{$this->config['type']}" . "/_search?search_type=query_then_fetch";
        } else {
            $url = "http://" . $this->config['elasticsearch_server'] . "/{$this->index}/_search?search_type=query_then_fetch";
        }
     ...
     */
    protected function esTotalDocumentCount() {
        $result = $this->esClient->indices();
        $indices = $result->indices;
        $total_docs = 0;
        if ($this->config['default_index'] == "_all") {
            foreach ($indices as $index) {
                $total_docs += $index->total->docs->count;
            }
        } else {
            $index = $this->config['default_index'];
            $total_docs = $indices->$index->docs->count;//num_docs
        }
        return $total_docs;
    ...
    /**
     * Query ElasticSearch server to get a list of all indexes available for
     * searching.
     *
    ...
    protected function getAllIndices() {
        $indices = [];
        $result = $this->esClient->stats();
        if (!$result || isset($result->error)) {
            return $indices;
        }
        if ($result && is_object($result)) {
            foreach ($result->indices as $name => $index) {
                $indices[] = $name;
            }
        }
        return $indices;
